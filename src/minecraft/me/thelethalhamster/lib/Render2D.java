...
public class Render2D{
	...
	public void drawItem(int x, int y, ItemStack stack){
		itemRenderer.renderItemIntoGUI(mc.fontRenderer, mc.getTextureManager(), stack, x, y);
	...
	public void drawTag(String s, double d, double d1, double d2) {
		...
		float f = 5;
		Wrapper.getWrapper().getEntityRenderer().disableLightmap(0D);
		d += 0.5D;
		d2 += 0.5D;
		FontRenderer fontrenderer = Wrapper.getWrapper().getFontRenderer();
		...
		GL11.glPopMatrix();
		Wrapper.getWrapper().getEntityRenderer().enableLightmap(0D);
	}
}
