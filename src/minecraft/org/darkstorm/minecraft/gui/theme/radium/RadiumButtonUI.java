...
public class RadiumButtonUI extends AbstractComponentUI<Button> {
	private final RadiumTheme theme;

	RadiumButtonUI(RadiumTheme theme) {
		super(Button.class);
		this.theme = theme;
	...
	protected void renderComponent(Button button) {
		...
		if(!button.isModuleEnabled()){
			...
			GuiUtil.getInstance().drawBorderedRect(0, 0, area.width, area.height, 0x908F0000, 0x908F0000);
		}
		...
		if(area.contains(mouse)) {
			GuiUtil.getInstance().drawBorderedRect(0, 0, area.width, area.height, 0x909A1919, 0x909A1919);
		}
