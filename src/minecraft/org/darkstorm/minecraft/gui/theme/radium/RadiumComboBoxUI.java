...
public class RadiumComboBoxUI extends AbstractComponentUI<ComboBox> {
	private final RadiumTheme theme;

	RadiumComboBoxUI(RadiumTheme theme) {
	...
	protected void renderComponent(ComboBox component) {
		...
		GuiUtil.getInstance().drawBorderedRect(0, 0, area.width, area.height + extendedHeight, 0x90000000, 0x90000000);
		...
		if(area.contains(mouse)) {
			...
			for(int i = 0; i < elements.length; i++) {
				...
				if(mouse.y >= area.y + offset
					...
					glEnd();
					GuiUtil.getInstance().drawBorderedRect(0, offset + height, area.width, area.height, 0x90000000, 0x90000000);
					break;
				}
				offset += height;
			}
